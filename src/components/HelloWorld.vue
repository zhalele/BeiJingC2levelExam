<template>
  <div v-if="list && !isEnd">
    ...
    <div class="sub-title">
      当前题目: <span class="weight">{{ count + 1 }}/{{ list.length }}</span>
    </div>
    ...
    <div>
      <el-button round type="info" size="large" style="display: block; margin: 0 auto" @click="submit"
      ...
      <div v-for="item in errList" class="box">
        ...
        <el-radio-group class="group-style" v-model="item.option.answer" disabled
          ...
          <div v-for="i in item.option.name">
            <div>
              <el-radio size="large" :value="i.split('.')[0]">
...
function changeB() {
  ...
  if (
    list.value[count.value].option.answer == "" ||
    (Array.isArray(list.value[count.value].option.answer) &&
      list.value[count.value].option.answer.length === 0)
  ) {
